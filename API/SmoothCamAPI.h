...
static_assert(0, "SmoothCamAPI: SKSE API type not defined. Define one of (SMOOTHCAM_API_SKSE, SMOOTHCAM_API_COMMONLIB)")
    ...
    public:
        ...
        /// </summary>
        /// <param name="myPluginHandle">Your assigned plugin handle</param>
        /// <param name="shouldMoveToGoal">Instructs the camera to move to it's goal position when control is gained
        /// back</param> <param name="moveNow">Move to the current goal immediately</param> <param name="ref">Object
        /// reference for rotation calculations</param> <returns>OK, NotOwner</returns>
    ...
    using InterfaceLoaderCallback = std::function<void(void* interfaceInstance, InterfaceVersion interfaceVersion)>;
    ...
    inline bool RequestInterface(SKSEMessagingInterface * skseMessaging, PluginHandle myPluginHandle,
        ...
        return skseMessaging->Dispatch(myPluginHandle, 0, &cmd, sizeof(PluginCommand), SmoothCamPluginName);
    };
    ...
    /// <returns></returns>
    [[nodiscard]]
    inline bool RegisterInterfaceLoaderCallback(SKSEMessagingInterface * skseMessaging, PluginHandle myPluginHandle,
        ...
        return skseMessaging->RegisterListener(
            myPluginHandle, SmoothCamPluginName, [](SKSEMessagingInterface::Message* msg) {
                ...
                switch (resp->type) {
                    case PluginResponse::Type::InterfaceProvider: {
                        ...
                        storedCallback(interfaceContainer->interfaceInstance, interfaceContainer->interfaceVersion);
                        break;
                    }
    ...
    inline bool RequestInterface(const SKSE::MessagingInterface* skseMessaging,
        ...
        return skseMessaging->Dispatch(0, &cmd, sizeof(PluginCommand), SmoothCamPluginName);
    }
    ...
    inline bool RegisterInterfaceLoaderCallback(const SKSE::MessagingInterface* skseMessaging,
        ...
        return skseMessaging->RegisterListener(SmoothCamPluginName, [](SKSE::MessagingInterface::Message* msg) {
            if (msg->sender && strcmp(msg->sender, SmoothCamPluginName) != 0) return;
            if (msg->type != 0) return;
            if (msg->dataLen != sizeof(PluginResponse)) return;
